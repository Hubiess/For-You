...
// Point density when there is only 1 image (highest)
const maxDensity = 15000;
// Pixel density when there are 10 or more images (lowest)
const minDensity = 4000;
// Maximum number of images we are interested in adjusting
...
const galaxyMaterial = new THREE.ShaderMaterial({
    ...
    vertexShader: `
        ...
        void main() {
            // Get the base color from geometry (same as vertexColors: true)
            vColor = color;
         ...
         // ---- RIPPLE EFFECT LOGIC ----
           ...
           // This line makes the particles smaller when they are far away, simulating the behavior of PointsMaterial
    ...
    fragmentShader: `
        varying vec3 vColor;
        void main() {
            // Make the particles round instead of square
...
// ---- CREATE HEART SHAPE POINT GROUPS ----
for (let group = 0; group < numGroups; group++) {
    ...
    // Geometry for camera proximity state
    ...
    // Geometry for camera distance state
    ...
    // Calculate the center of the point group and move it to the origin
    ...
    img.onload = () => {
        ...
        // Store states for later transitions
...
function createShootingStar() {
    ...
    planet.add(atmosphere); // Add atmosphere as child of planet
...
// Function to create texture for planet
...
// Shader for storm effect on planet surface
const stormShader = {
    uniforms: {
        time: { value: 0.0 },
...
function createTextRings() {
    ...
    for (let i = 0; i < numRings; i++) {
        const text = ringTexts[i % ringTexts.length] + '   '; // Add space
        const ringRadius = baseRingRadius + i * ringSpacing;

        // ---- Logic to analyze and adjust font size (retained) ----
        ...
        ringGroup.userData = {
            ...
            speed: 0.008, // Rotation speed
            tiltSpeed: 0,
            rollSpeed: 0,
            pitchSpeed: 0, // Shaking speed
            tiltAmplitude: Math.PI / 3,
            rollAmplitude: Math.PI / 6,
            pitchAmplitude: Math.PI / 8, // Shaking amplitude
            tiltPhase: Math.PI * 2,
            rollPhase: Math.PI * 2,
            pitchPhase: Math.PI * 2, // Shake
            isTextRing: true
        };
...
function animatePlanetSystem() {
    if (window.textRings) {
        ...
        window.textRings.forEach((ringGroup, index) => {
            ...
            const pulse = (Math.sin(time * 1.5 + index) + 1) / 2; // value from 0 to 1
            const textMesh = ringGroup.children[0];
            if (textMesh && textMesh.material) {
                // Change opacity from 0.7 to 1.0
...
// =========================================================================
// ---- ADD HINT EFFECT BY CLICKING ON THE ORB (HINT ICON) ----
// =======================================================================

let hintIcon;
let hintText;
/**
 * Create a 3D mouse cursor icon to hint the user.
 * FINAL VERSION: The cursor is solid white and is positioned
 * further away from the center sphere.
 */
function createHintIcon() {
    ...
    // --- 1. CREATE CURSOR SHAPE (Keep it the same) ---
    ...
    // --- 2. CREATE WHITE CURSOR ---

    // Background layer (previously black border, now white background)
    const backgroundGeometry = new THREE.ShapeGeometry(cursorShape);
    const backgroundMaterial = new THREE.MeshBasicMaterial({
        color: 0xffffff, // CHANGE: Change border to white
        side: THREE.DoubleSide
    });
    const backgroundMesh = new THREE.Mesh(backgroundGeometry, backgroundMaterial);

    // White layer inside (not needed now but keep for thickness)
    const foregroundGeometry = new THREE.ShapeGeometry(cursorShape);
    const foregroundMaterial = new THREE.MeshBasicMaterial({
        color: 0xffffff, // Keep the color white
        side: THREE.DoubleSide
    });
    ...
    // --- 3. CREATE CIRCLE AROUND (Keep it the same) ---
    ...
    // CHANGE: Place icon further away
    hintIcon.position.set(1.5, 1.5, 15); // Increase Z value from 12 to 20
...
function animateHintIcon(time) {
    ...
    if (!introStarted) {
        ...
        // Process the suggested text (add new effects)
        if (hintText) {
        ...
        // Hide icon when intro has started
...
// ---- EDIT ANIMATE LOOP ----
// You need to replace the old animate() function with this modified function.
function animate() {
    ...
    // Material conversion logic for heart point groups
    scene.traverse(obj => {
...
// ---- EVENT HANDLING AND STARTUP FUNCTIONS ----
createShootingStar();
createHintIcon(); // Call the icon creation function
createHintText();
...
// Limit the number of stars displayed initially
...
function onCanvasClick(event) {
    ...
    if (intersects.length > 0) {
        ...
        // --- AUTOPLAY ERROR FIXING LOGIC ---
        if (window.musicManager) {
            // Try to play music immediately
            window.musicManager.play().catch(error => {
                // If the browser blocks (NotAllowedError), do nothing here. 
                // // The togglePlayback button will handle this later.
                console.warn("Putar otomatis diblokir, pengguna perlu berinteraksi dengan tombol suara.", error);

                // Important: Marks that the user intended to turn on the music. 
                // // This makes the toggle button work correctly on the first press.
                if (window.musicManager.audio) {
                    // We don't actually turn it off, just update the UI so it looks like it's turned off
                    // and wait for the user to press the button.
                    window.musicManager.audio.muted = true; // Temporarily mute
                    window.musicManager.updateUI(); // Update icon
                }
            });
        } else {
            console.error("musicManager belum diinisialisasi!");
        }
...
// ---- SETTINGS FOR INTERFACE AND MOBILE ----
...
// =========================================================================
// ---- CHECK SCREEN ORIENTATION TO DISPLAY WARNING ----
// =====================================================================

function checkOrientation() {
    // Check if height is greater than width (portrait screen on phone)
    // Add a small condition to not trigger on narrow desktop screens.
...
// Listen for events to recheck screen orientation
window.addEventListener('DOMContentLoaded', checkOrientation);
window.addEventListener('resize', checkOrientation);
window.addEventListener('orientationchange', () => {
    // Add delay for browser to update correct size
